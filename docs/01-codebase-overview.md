...
Requra.AI is a Python 3.11 AI-processing microservice. This repository does not contain the product frontend, the .NET/backend domain service, or the backend's primary object storage. Those systems call this service through the internal API and consume its structured result.

The service accepts inline text/transcripts, multi-document files, and backend-owned document/audio references. It validates and normalizes the input, runs a 13-node LangGraph workflow, stores job artifacts, and exposes status/result APIs.
...
│   │   ├── graph/            LangGraph construction and routers (13 active nodes)
│   │   ├── maintenance/      retention cleanup commands and service
│   │   ├── nodes/            13 executable graph nodes and processing helpers
...
│   │   ├── services/         file inspection, fingerprinting, source processing, quality scoring
│   │   ├── validators/       story validation
│   │   └── main.py           FastAPI application and demo routes
│   ├── migrations/           Alembic schema history
│   ├── scripts/              diagnostics, simulation, and readiness evaluation suite
│   ├── tests/                unit, API, worker, contract, security, and integration tests
...
│   └── reports/              real provider readiness evaluation reports
...
| Source processing | Heterogeneous doc extraction, duration checks, STT, PII masking, chunking | `ai-service/app/nodes/prepare_sources.py`, `ai-service/app/services/source_processing/` |
| AI stages | Extraction, deduplication, classification, grounding, generation, quality, summary, format | `ai-service/app/nodes/*.py` |
| Providers | Chat model fallback, speech-to-text, and embeddings | `ai-service/app/llm.py`, `ai-service/app/services/source_processing/audio.py`, `ai-service/app/rag/embeddings.py` |
...
| Maintenance | Database retention cleanup for results, chunks, and embeddings | `ai-service/app/maintenance/cleanup.py` |

## Important entry points

- `ai-service/app/main.py` → `app`, `POST /process`, `POST /process-json`, `GET /status/{job_id}`, `/health`, and `/ready`.
- `ai-service/app/api/internal.py` → protected `/internal/*` routes (`POST /internal/jobs`, `POST /internal/process`, `POST /internal/stories/regenerate`).
- `ai-service/app/graph/pipeline.py` → `build_pipeline()` and exported `graph` (13 registered nodes).
...
- `ai-service/app/worker/state.py` → `build_worker_initial_state()` and Redis/backend input reconstruction.
...
- `ai-service/app/maintenance/cleanup.py` → `run_retention_cleanup()` CLI and service.
