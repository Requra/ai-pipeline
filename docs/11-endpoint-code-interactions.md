...
This document details the code-level implementation of the FastAPI endpoints, the step-by-step functionality of each handler, and how the files in the repository interact to orchestrate request and background processing flows.
...
| **Graph Router** | [graph/pipeline.py](../ai-service/app/graph/pipeline.py) | Defines `build_pipeline()`, compiles the 13-node LangGraph workflow, and registers conditional routes. |
| **DB Repository** | [repositories.py](../ai-service/app/store/db/repositories.py) | Encapsulates SQL transactions (SQLAlchemy) against PostgreSQL (represented by [models.py](../ai-service/app/store/models.py)). |
...
*   **Readiness Check (`GET /ready`)**
    ...
    *   *Details*: Calls `build_readiness_report()` from [app/startup.py](../ai-service/app/startup.py). Pings PostgreSQL and Redis. If any core connection is broken, it raises an HTTP `503 Service Unavailable`.

### Public Demo Endpoints ([app/main.py](../ai-service/app/main.py))

*   **Submit Demo JSON (`POST /process-json`)**
    *   *Code Function*: `process_json()`
    *   *Details*: Validates input text. Calls `_create_and_dispatch_demo_job()`, which checks the local status cache and dispatches the job.
*   **Upload Demo Document (`POST /process`)**
    *   *Code Function*: `process_document()`
    *   *Details*: Receives file bytes via `multipart/form-data`. Calls `detect_mime_and_type()` to inspect file headers, enforces size limits, and enqueues the job.
...
*   **Retry Job (`POST /internal/jobs/{job_id}/retry`)**
    ...
    *   *Details*: Evaluates if current job status is `FAILED` or `CANCELLED`. If so, increments the database `attempt_number`, resets status to `QUEUED`, and enqueues the job back to the RQ worker queue.

---

## Production endpoint and file-transport guide

This section is the canonical explanation of how a backend integration supplies text, uploaded bytes, or backend-owned document references.
...
|---|---|---:|---:|---:|---:|---|
| `POST /internal/jobs` | Internal bearer token | No | Yes, for `text` and `backend_transcript` | Yes, for `backend_document`, `backend_audio`, and `backend_sources` | Yes | Canonical production job API. |
| `POST /internal/process` | Internal bearer token | Yes, multipart | No | Creates a source reference for uploaded files | Not required for submitted files | Compatibility upload endpoint. |
| `POST /internal/process-json` | Internal bearer token | No | Yes | Optional metadata only | Used if worker recovers missing transcript text | Compatibility text/transcript endpoint. |
...
The internal routes are mounted in `app/api/internal.py`; request models are in `app/api/schemas.py`.

### Option A: backend-owned storage with document references

This is the recommended production integration for documents and audio. The backend keeps ownership of the original file and sends metadata rather than copying the file through the job-creation request.
...
The `file_url` may be omitted when the AI service has `BACKEND_BASE_URL` and `BACKEND_SERVICE_TOKEN` configured. The worker then requests `{BACKEND_BASE_URL}/internal/documents/{document_id}/content`.
...
`BackendDocumentClient.fetch_document_bytes()` validates the URL, blocks unsafe hosts and credentials in URLs, limits size, handles redirects, and verifies the hash when one is supplied. Custom storage domains must be included in `ALLOWED_DOWNLOAD_DOMAINS` unless they match the configured backend origin.

### Option C: multipart upload to the AI service

`POST /internal/process` accepts file bytes directly. The legacy single-file contract remains available as `file` plus optional `document_id`. For a multi-document or mixed document/audio job, repeat `files` and repeat `document_ids` in matching order:

```text
multipart/form-data
  files        = requirements.pdf
  files        = meeting-audio.mp3
  job_id       = upload-001
  project_id   = project-1
  tenant_id    = tenant-1
  document_ids = doc-789
  document_ids = audio-790
```

The server path carries `raw_inputs` (one independently identified byte stream per upload) into `prepare_sources`, which handles PDF/DOCX extraction, audio duration checks, speech-to-text, PII masking, and chunking in parallel.
...
For an already-transcribed meeting, use `backend_transcript` with `content`. No file URL is required. The content is placed in `raw_text` and passes through source preparation without calling speech-to-text.
...
[app/worker/main.py] (worker daemon loop)
      ...
      └──> Invokes [app/worker/runner.py::run_job_entry]
                 ...
                 │          │
                 │          └──> Recovers raw content (from Redis cache or S2S downloads)
                 │
                 ├──> Calls [app/graph/pipeline.py::build_pipeline] (compiles LangGraph)
                 │
                 ├──> Executes graph.ainvoke(initial_state)
                 │          │
                 │          └──> State moves through the 13 active graph nodes:
                 │               1. detect_file_type.py
                 │               2. prepare_sources.py (Parallel doc extraction & STT)
                 │               3. build_source_index.py (BM25 + pgvector embeddings)
                 │               4. extract.py (LLM extraction via ResilientLLMClient)
                 │               5. dedupe_requirements.py
                 │               6. retrieve_evidence.py
                 │               7. classify.py
                 │               8. evidence_grounding.py
                 │               9. generate.py (generates user stories)
                 │              10. quality_gate.py (validates quality)
                 │              11. repair_stories.py (bounded repair loop)
                 │              12. summarize.py (builds executive summary)
                 │              13. format.py (serializes state to V1 JobResult)
                 │
                 ├──> Calls [app/store/db/repositories.py::save_job_results]
                 │          (Persists requirements, stories, and summary to DB)
                 │
                 ├──> Calls [app/store/db/repositories.py::mark_completed] (sets database status)
                 │
