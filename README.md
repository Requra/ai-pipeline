...
Requra.AI is an evidence-grounded requirements engineering microservice that transforms project documents, meeting audio, transcripts, and notes into structured requirements, delivery-ready user stories, Given–When–Then acceptance criteria, executive summaries, quality findings, and export data. Its production-shaped architecture decouples synchronous API request handling from asynchronous worker execution and preserves end-to-end evidence traceability back to the source phrases that justify every requirement.
...
<p align="center">
  ...
  <img alt="LangGraph" src="https://img.shields.io/badge/LangGraph-13--node_graph-6D5DFB">
...
```text
Unstructured project knowledge (PDF, DOCX, TXT, Audio, Transcripts)
                        ↓
     Unified source preparation & chunk corpus
                        ↓
...
The service extracts source-backed requirements, merges duplicates, retrieves and verifies supporting quotes against original chunks, classifies requirement intent, maps requirements to user stories, validates story quality, and exposes the result through a versioned API contract.
...
| PDF and DOCX documents | Extraction, normalization, and optional PII masking | Verbatim source references and retrieval signals |
| Backend transcripts | Requirement extraction and deduplication | User stories with flexible requirement coverage |
| Meeting audio | Bounded duration check + Groq/Deepgram speech-to-text | Given–When–Then acceptance criteria |
| Mixed documents + audio | Bounded concurrent preparation behind `prepare_sources` | Unified chunk corpus with preserved provenance |
| Backend-owned document/audio references | BM25 evidence retrieval; optional embeddings & hybrid search | Quality issues, warnings, and aggregate quality report |
...
Backend transcripts enter the text path and **do not call speech-to-text**. Backend audio references and audio uploads enter the speech-to-text path with duration limits and automatic fallback. When multiple heterogeneous documents and audio files are submitted together, `prepare_sources` processes them concurrently and converges them into one unified chunk corpus.

The requirement model distinguishes functional requirements, non-functional requirements, business rules, constraints, assumptions, open questions, and out-of-scope items. Extraction carries inferred priority and confidence; exact and near duplicates are merged; optional requirement embeddings identify semantic conflict candidates; and coverage supports one-to-one, one-to-many, many-to-one, acceptance-criteria-only, non-story, and needs-review outcomes.

Generated stories normalize actors toward a singular agile persona and retain source requirement IDs and evidence quotes. The validator checks a minimum-of-two acceptance-criteria rule, flags generic or missing criteria and weak story structure, detects duplicate story IDs, and feeds quality findings into the optional bounded repair-and-revalidate pass.

## Architecture at a glance

```mermaid
flowchart LR
    Caller["Backend / Client"] --> API["FastAPI API\n/internal/jobs"]
    API --> JobStore[("PostgreSQL\nJobs, Events, Results")]
    API --> RedisQ["Redis / RQ Queue\nTransient Input Cache"]
    RedisQ --> Worker["RQ Worker Process\napp.worker.main"]
    Worker --> Recovery["Input Recovery\nRedis Cache or Backend S2S"]
    Recovery --> Pipeline["13-Node LangGraph\nUnified PipelineState"]
    Pipeline --> Providers["AI Providers\nGroq / OpenRouter / Deepgram"]
    Pipeline --> PGVector[("PostgreSQL + pgvector\nChunks & Embeddings")]
    Worker --> Callback["Allowlisted Webhook\nOptional Best-Effort"]
    Caller --> Probes["/health & /ready"]
```

The API validates and fingerprints a request, creates its durable job record, and dispatches work. The worker reconstructs input, executes the graph, persists artifacts, updates durable status, and only then attempts an optional callback.

- **Separated execution:** FastAPI and the RQ worker run as distinct services, allowing request ingestion and heavy graph execution to scale independently.
- **Durable path:** PostgreSQL stores jobs, attempts, events, source metadata/chunks, decomposed results, quality reports, and final result contracts. pgvector stores optional chunk embeddings.
- **Transient path:** Redis/RQ dispatches jobs and caches inline input (with a 6-hour TTL). Redis is not the authoritative store; the in-process queue serves as a local/test fallback.
- **Result delivery:** Callers poll durable status and result endpoints. An optional callback is best-effort, origin-restricted, and executed after persistence.

See [System architecture](docs/03-system-architecture.md) and [Database and storage](docs/06-database-and-storage.md).

## The 13-node pipeline

The active graph in [`app/graph/pipeline.py`](ai-service/app/graph/pipeline.py) registers the **13 active nodes** below. Modality-specific ingestion, audio transcription, PII redaction, and chunk parsing are encapsulated behind `prepare_sources`. Its recursion limit of 60 is a LangGraph execution-step budget. The only cycle is the configuration- and attempt-bounded story repair loop.
...
|---:|---|---|---|---|
| 1 | `detect_file_type` | Inspect bytes, signatures, declared media type, size, and source metadata | Validated file type and metadata | [`detect_file_type.py`](ai-service/app/nodes/detect_file_type.py) |
| 2 | `prepare_sources` | Process heterogeneous documents/audio in parallel; extract, transcribe, mask PII, chunk, and check relevance | Unified chunk corpus with provenance | [`prepare_sources.py`](ai-service/app/nodes/prepare_sources.py) |
| 3 | `build_source_index` | Build per-job BM25 index; optionally generate and persist chunk embeddings | In-memory lexical index handle and stats | [`build_source_index.py`](ai-service/app/nodes/build_source_index.py) |
| 4 | `extract` | Parse structured requirement candidates with evidence spans, priority, and confidence | Extracted requirements | [`extract.py`](ai-service/app/nodes/extract.py) |
| 5 | `dedupe_requirements` | Merge exact/near duplicates; optionally detect semantic conflicts | Canonical deduplicated requirements | [`dedupe_requirements.py`](ai-service/app/nodes/dedupe_requirements.py) |
| 6 | `retrieve_evidence` | Retrieve lexical (BM25) or optional hybrid vector support and record scores | Enriched evidence candidates | [`retrieve_evidence.py`](ai-service/app/nodes/retrieve_evidence.py) |
| 7 | `classify` | Assign functional, non-functional, business rule, and special category labels | Classified requirements | [`classify.py`](ai-service/app/nodes/classify.py) |
| 8 | `evidence_grounding` | Verify that evidence quotes are non-empty and occur verbatim in source chunks | Verified evidence & grounding issues | [`evidence_grounding.py`](ai-service/app/nodes/evidence_grounding.py) |
| 9 | `generate` | Generate structured user stories, Given–When–Then criteria, and coverage mappings | User stories | [`generate.py`](ai-service/app/nodes/generate.py) |
| 10 | `quality_gate` | Validate stories, acceptance criteria, duplicates, and compute quality report | Quality report and issues | [`quality_gate.py`](ai-service/app/nodes/quality_gate.py) |
| 11 | `repair_stories` | Repair repairable story-quality failures and return for revalidation | Repaired stories (bounded loop) | [`repair_stories.py`](ai-service/app/nodes/repair_stories.py) |
| 12 | `summarize` | Produce a bounded structured digest of decisions, risks, assumptions, and scope | Structured summary | [`summarize.py`](ai-service/app/nodes/summarize.py) |
| 13 | `format` | Map internal state to the public V1 `JobResult` contract and export rows | Serialized `JobResult` | [`format.py`](ai-service/app/nodes/format.py) |

Routing is explicit: if source preparation encounters an irrecoverable error or all sources are rejected as irrelevant, the graph short-circuits directly from `prepare_sources` to `format`. `quality_gate` routes to `repair_stories` only when repair is enabled, attempts remain, and an active story has a repairable rule violation. The repaired stories are re-evaluated by `quality_gate`.
...
```mermaid
flowchart LR
    A["Source Document / Audio"] --> B["Source Chunk\npage / time / speaker"]
    B --> C["Retrieved Evidence Quote\nverbatim check"]
    C --> D["Classified Requirement\nFR / NFR / BR"]
    D --> E["Generated User Story\nAgile Persona"]
    E --> F["Acceptance Criteria\nGiven-When-Then"]
    F --> G["Public Result source_refs\nFull Provenance Chain"]
```

Each source is divided into chunks that retain origin metadata (document ID, page number, speaker, timestamp, or character offsets). Extraction attaches evidence spans; retrieval adds relevant source snippets and scores; grounding separately verifies quotes against the source text. Requirements and generated stories carry those evidence spans into `source_refs` in the public V1 result.

Retrieval is not treated as proof by itself. BM25 and optional vector similarity improve recall, but the grounding node separately verifies quotes against source chunks. The RAG layer is purpose-built for requirements traceability—not a general conversational chatbot.
...
The selected OpenRouter, OpenAI, or Groq chat provider can use a configured fallback chain with retryable-error handling. Nodes use structured parsing, Pydantic validation, JSON extraction/repair, and provider/model/latency/token metadata. STT supports Groq or Deepgram with automatic fallback; embeddings support OpenAI-compatible endpoints.
...
A canonical request fingerprint distinguishes identical duplicate submissions from reused job IDs with modified content. Matching active requests return the existing job status without duplicate execution; mismatched payloads return `409 Conflict`; failed/cancelled jobs can create a new attempt under explicit retry rules.
...
The complete schema also includes source documents, structured summary fields, quality findings, Excel/Jira export rows, artifacts, warnings, processing time, and structured pipeline errors. See [`app/schemas/items.py`](ai-service/app/schemas/items.py) and the canonical [OpenAPI artifact](docs/openapi.json).
...
sequenceDiagram
    ...
    participant G as 13-Node LangGraph
    participant P as AI Providers (LLM / STT)
    participant B as Backend Callback

    C->>A: POST /internal/jobs (Bearer Token)
    A->>A: Validate request & compute fingerprint
    A->>S: Persist QUEUED job record & sources
    A->>Q: Enqueue job ID & cache transient input
    A-->>C: 202 Accepted {job_id, status: QUEUED}
    Q->>W: Dequeue job ID
    W->>S: Mark PROCESSING & load metadata
    W->>W: Recover sources (Redis cache or backend S2S)
    W->>G: Execute 13-node pipeline
    G->>P: Bounded LLM / STT / embedding calls
    P-->>G: Structured provider outputs
    G-->>W: Final JobResult payload
    W->>S: Persist chunks, decomposed entities, result, events
    opt Allowlisted callback configured
        W->>B: Post best-effort terminal webhook
    end
    loop Polling until terminal
        C->>A: GET /internal/jobs/{job_id}
        A->>S: Query status & progress
        A-->>C: 200 OK {status, progress_pct}
    end
    C->>A: GET /internal/jobs/{job_id}/result
    A->>S: Read persisted result
    A-->>C: 200 OK {JobResult}
```
...
`POST /internal/process` accepts the legacy single `file` field and repeated `files` fields. For multiple uploads, repeat `files` and repeat `document_ids` in matching order. The endpoint supports mixed documents and audio in the same job, persists one source manifest row per file, and preserves per-document chunk provenance.

See [API and data flow](docs/05-api-and-data-flow.md), [endpoint code interactions](docs/11-endpoint-code-interactions.md), and the canonical [OpenAPI artifact](docs/openapi.json).

## Quick start

### Minimal local development

Requirements: Poetry and Python `^3.11`.
...
See [Local development](docs/02-local-development.md), [Deployment and operations](docs/08-deployment-and-operations.md), and [Security and configuration](docs/09-security-and-configuration.md).
...
Verified with the repository’s default test suite:
...
| Graph and nodes | [`tests/test_pipeline.py`](ai-service/tests/test_pipeline.py), [`tests/nodes/`](ai-service/tests/nodes/) | 13-node execution, source prep, parsing, extraction, classification, grounding, generation, quality, repair, summary, format |
| Retrieval | [`tests/rag/`](ai-service/tests/rag/) | BM25 scoring, hybrid merge, vector augmentation behavior |
| Providers and fallbacks | [`test_llm_provider.py`](ai-service/tests/test_llm_provider.py), [`test_llm_fallback.py`](ai-service/tests/test_llm_fallback.py) | Provider selection, retry/fallback behavior, metadata, STT paths |
...
| Persistence | [`tests/store/`](ai-service/tests/store/) | Memory stores and database repository behavior with pool configuration |
| Maintenance | [`tests/maintenance/`](ai-service/tests/maintenance/) | Retention cleanup command for results, chunks, and embeddings |
...
See [Testing, debugging, and observability](docs/07-testing-debugging-and-observability.md).
...
│   │   ├── maintenance/  # retention cleanup commands and service
│   │   ├── nodes/        # the 13 executable pipeline nodes & helpers
...
│   │   ├── services/     # source processing, fingerprinting, quality scoring
...
│   └── scripts/          # diagnostics, simulation, and readiness suite
├── docs/                 # canonical engineering documentation
│   ├── openapi.json      # canonical generated OpenAPI contract
│   └── reports/          # real provider readiness evaluations
...
| Backend integrator | [API and data flow](docs/05-api-and-data-flow.md) → [Endpoint interactions](docs/11-endpoint-code-interactions.md) → [OpenAPI](docs/openapi.json) |
