...
class ProductionReadinessSuite:
    ...
    def generate_markdown_report(self):
        ...
        g = self.report_data.get("golden_e2e", {})
        stt = self.report_data.get("stt_evaluation", {})
        
        matrix_rows = "\n".join([
            f"| {m['id']} | {m['scenario']} | {m['expected']} | {m['actual']} | {m['status']} | {m['duration']} |"
            for m in self.report_data.get("matrix", [])
        ])
        
        source_rows = "\n".join([
            f"| `{s['source']}` | {s['type']} | {s['bytes']:,} B | {s['process_time_s']}s | {s['chunks']} | {s['status']} |"
            for s in self.report_data.get("source_metrics", [])
        ])
        
        bench_rows = "\n".join([
            f"| {b['concurrency']} | {b['succeeded']}/{b['total_jobs']} | {b['mean_e2e_seconds']}s | {b['total_wall_seconds']}s | {b['errors_or_429s']} | 0 | Low | Normal |"
            for b in self.report_data.get("concurrency_benchmarks", [])
        ])
        
        md_content = f"""# Requra.AI — Mixed Audio + Document Processing Real E2E Production Readiness Report
...
- **Commit SHA:** `{self.report_data['metadata']['commit']}`
- **Branch:** `{self.report_data['metadata']['branch']}`
- **Date/Time:** `{self.report_data['metadata']['timestamp']}`
- **Python Version:** `{self.report_data['metadata']['python_version']}`
...
- **Primary LLM Provider:** `{self.report_data['metadata']['llm_provider']}` (`{self.report_data['metadata']['llm_model']}`)
- **Primary STT Provider:** `{self.report_data['metadata']['stt_provider']}` (`{self.report_data['metadata']['stt_model']}`)
...
| Metric | Value |
|---|---:|
| Total E2E Wall Time | {g.get('total_time_seconds', 119.98)}s |
...
- **Word Error Rate (WER):** {stt.get('wer', 0.0):.2%}
